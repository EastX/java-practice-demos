...
**核心类**
- Redis 配置 [`ExpandRedisConfig.java`](./src/main/java/cn/eastx/practice/demo/cache/config/spring/ExpandRedisConfig.java)
    - 注意开启 Spring Cache 需要在配置类（或启动类）上增加 `@EnableCaching`
    - 核心处理：注入缓存管理器及配置处理缓存文件中的缓存时长
        ```java
        ...
        ```
    - 配置缓存空间名称对应时长 [`application-custom.yml`](./src/main/resources/application-custom.yml)
        ```yml
...
- 自定义 Redis 缓存管理器 [`ExpandRedisCacheManager.java`](./src/main/java/cn/eastx/practice/demo/cache/config/spring/ExpandRedisCacheManager.java)
    - 核心处理：重写创建缓存执行器逻辑，支持缓存空间名称中附带缓存时间根据指定符号分隔
        ```java
        @Override
...
- 自定义缓存注解逻辑实现
    ...
    - 自定义缓存 key 生成器 [`ExpandKeyGenerator.java`](./src/main/java/cn/eastx/practice/demo/cache/config/spring/ExpandKeyGenerator.java)
        - 与自定义缓存注解 `spelKey()` 搭配处理，注意使用后初始配置的缓存空间前缀仍会生效 `RedisCacheConfiguration.prefixCacheNameWith("spring:cache:")`
        - 有需要则在 `ExpandRedisConfig` 中进行注入
        - 核心处理：支持 SpEL key 格式解析
            ```java
            @Override
            ...
            ```
    - 自定义缓存注解过期时间初始化配置类（可选） [`ExpandCacheExpireConfig.java`](./src/main/java/cn/eastx/practice/demo/cache/config/spring/ExpandCacheExpireConfig.java)
        - 与自定义缓存注解 `timeout() + unit()` 对应处理
        - 有需要则在 ExpandRedisConfig 中进行注入
        - 核心处理：利用 Spring Component Bean 获取到使用 `@ExpandCacheable` 注解的方法，利用反射获取注解属性并设置缓存空间过期时间；Map 处理，统一名称缓存空间将会出现替换情景
            ```java
...
- Spring Cache 整合 Redis 处理示例 Controller [`ExpandSpringCacheController.java`](./src/main/java/cn/eastx/practice/demo/cache/controller/ExpandSpringCacheController.java)
    - 示例代码
        ...
        // 使用 @Cacheable 
        @GetMapping("/spel2")
        @Cacheable(cacheNames = "prefix#5m", cacheManager = "ExpandRedisCacheManager", keyGenerator = "ExpandKeyGenerator")
        ...
        // 使用 @ExpandCacheable
        @GetMapping("/spel3")
        @ExpandCacheable(cacheNames = "prefix", spelKey = "hello-spel3-${#id}", timeout = 100, unit = TimeUnit.SECONDS)
...
- 优点
    - 使用 Spring 自带功能，通用性强
- 缺点
    - 针对缓存空间处理缓存时长，缓存时间一致可能导致缓存雪崩，自定义处理需要理解相应实现
